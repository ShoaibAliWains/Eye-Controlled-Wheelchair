...
class MotorController:
    ...
    def set_target(self, direction, speed=None):
        ...
        # FIX FOR EXHIBITION: Sharp zero-radius turns so it's visibly obvious!
        if direction == "FORWARD":
            self.target_speed_l = speed
            self.target_speed_r = speed
        elif direction == "LEFT":
            self.target_speed_l = 0.0     # 0% completely stops left motor
            self.target_speed_r = speed
        elif direction == "RIGHT":
            self.target_speed_l = speed
            self.target_speed_r = 0.0     # 0% completely stops right motor
        else: # STOP
